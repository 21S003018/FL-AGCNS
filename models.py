...
class SuperNet(nn.Module, Structure):
    ...
    def forward(self,x,edge_index,supermask):
        x = self.x_blink(x) if supermask[0] == 0 else eval('self.x_{}'.format(supermask[0]))(eval('self.x{}'.format(supermask[0]))(x))
        if supermask[1] == 0:
            return self.z_blink(x) if supermask[7] == 0 else eval('self.z_{}'.format(supermask[7]))(
                eval('self.z{}'.format(supermask[7]))(x))
        ...
        l2_input = x if (1 <= supermask[2] and supermask[2] <= 12) else l1_output
        l2_output = eval('self.y{}2_{}'.format(int((supermask[2] - 1)/12),(supermask[2] - 1) % 12 + 1))(l2_input,edge_index)
        if supermask[3] == 0:
            x = (l1_output+l2_output)/2
            return self.z_blink(x) if supermask[7] == 0 else eval('self.z_{}'.format(supermask[7]))(
                eval('self.z{}'.format(supermask[7]))(x))
        ...
        l3_input = x if (1 <= supermask[3] and supermask[3] <= 12) else (l1_output if (13 <= supermask[3] and supermask[3] <= 24) else l2_output)
        l3_output = eval('self.y{}3_{}'.format(int((supermask[3] - 1)/12),(supermask[3] - 1) % 12 + 1))(l3_input,edge_index)
        if supermask[4] == 0:
            x = (l1_output+l2_output+l3_output)/3
            return self.z_blink(x) if supermask[7] == 0 else eval('self.z_{}'.format(supermask[7]))(
                eval('self.z{}'.format(supermask[7]))(x))

        l4_input = x if (supermask[4] in [1,12]) else (l1_output if (supermask[4] in [12+1,12*2]) else (l2_output if (supermask[4] in [24+1,12*3]) else l3_output))
        l4_output = eval('self.y{}4_{}'.format(int((supermask[4] - 1)/12),(supermask[4] - 1) % 12 + 1))(l4_input,edge_index)
        if supermask[5] == 0:
            x = (l1_output+l2_output+l3_output+l4_output)/4
            return self.z_blink(x) if supermask[7] == 0 else eval('self.z_{}'.format(supermask[7]))(
                eval('self.z{}'.format(supermask[7]))(x))

        l5_input = x if (supermask[5] in [1,12]) else (l1_output if (supermask[5] in [12+1,12*2]) else (l2_output if (supermask[5] in [24+1,12*3]) else (l3_output if (supermask[5] in [36+1,12*4]) else l4_output)))
        l5_output = eval('self.y{}5_{}'.format(int((supermask[5] - 1)/12),(supermask[5] - 1) % 12 + 1))(l5_input,edge_index)
        if supermask[6] == 0:
            x = (l1_output+l2_output+l3_output+l4_output+l5_output)/5
            return self.z_blink(x) if supermask[7] == 0 else eval('self.z_{}'.format(supermask[7]))(
                eval('self.z{}'.format(supermask[7]))(x))

        l6_input = x if (supermask[6] in [1,12]) else (l1_output if (supermask[6] in [12+1,12*2]) else (l2_output if (supermask[6] in [24+1,12*3]) else (l3_output if (supermask[6] in [36+1,12*4]) else (l4_output if (supermask[6] in [48+1,12*5]) else l5_output))))
        l6_output = eval('self.y{}6_{}'.format(int((supermask[6] - 1)/12),(supermask[6] - 1) % 12 + 1))(l6_input,edge_index)
        x = (l1_output + l2_output + l3_output + l4_output + l5_output + l6_output) / 6
...
class Darts(nn.Module, Structure):
    def __init__(self, nfeat, nclass):
        ...
        self.tau = 10
        self.register_parameter('alpha',nn.Parameter(Variable(torch.ones(5)/2,requires_grad=True)))
        self.register_parameter('gamma',nn.Parameter(Variable(torch.ones(5)/2,requires_grad=True)))
        for j in range(1,7):
            self.register_parameter('beta{}'.format(j),nn.Parameter(eval('Variable(torch.ones({})/2,requires_grad=True)'.format(j*12+1))))
        return
    ...
    def forward(self, x, edge_index):
        ...
        for i in range(5):
            tmp_lis.append(eval('tmp_alpha[{}] * self.x_{}(self.x{}(x))'.format(i,i+1,i+1)))
        ...
        for j in range(1,6 + 1):
            ...
            for i in range(j):
                exec('tmp_beta{}[{}:{}] = F.softmax(self.beta{}[{}:{}])'.format(j,i*12+1,i*12+13,j,i*12+1,i*12+13))
            for i in range(1,j*12+1):
                tmp_lis.append(eval('self.y{}{}_{}(y{},edge_index)*tmp_beta{}[i]'.format(int((i-1)/12),j,(i-1)%12+1,int((i-1)/12),j)))
            exec('y{} = torch.sum(torch.stack(tmp_lis,axis=0),dim=0) + 0*self.beta{}[0]**2'.format(j,j))
        x = eval('y1') + eval('y2') + eval('y3') + eval('y4') + eval('y5') + eval('y6')
        tmp_lis = []
        tmp_gamma = F.softmax(self.gamma)
        for i in range(5):
            tmp_lis.append(eval('tmp_gamma[{}] * self.z_{}(self.z{}(x))'.format(i, i + 1, i + 1)))
        x = torch.sum(torch.stack(tmp_lis, axis=0), dim=0)
        return x
